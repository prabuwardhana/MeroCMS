...
type FileActions = {
  ...
  updateUploadStatus: (id: string, uploadStatus: ExtendedFile["uploadStatus"]) => void;
...
export const useFileUploadStore = create<FileUploadStore>()((set) => ({
  ...
  appendFiles: (acceptedFiles) =>
    set((state) => {
      const notDuplicatedNewFiles: ExtendedFile[] = acceptedFiles
        .filter((file) => {
          const isDuplicate = state.files.some((subItem) => subItem.id === `${file.name}${file.size}`);
          return !isDuplicate;
        })
        .map((file) => ({
          file,
  ...
  updateUploadProgress: (id, uploadProgress) =>
    set((state) => ({
      files: state.files.map((file) => (file.id === id ? { ...file, uploadProgress } : file)),
    })),

  updateUploadStatus: (id, uploadStatus) =>
    set((state) => ({
      files: state.files.map((file) => (file.id === id ? { ...file, uploadStatus } : file)),
    })),
